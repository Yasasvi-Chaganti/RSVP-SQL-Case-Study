...
-- Type your code below:
SELECT 
    genre, year, COUNT(M.id) AS movie_count
FROM
    genre G
        INNER JOIN
...
WITH uni_genre AS 
(
SELECT movie_id, count(genre) as number_of_genres FROM genre
GROUP BY movie_id HAVING number_of_genres = 1
)
SELECT count(movie_id) AS no_of_movies FROM uni_genre;
...
SELECT 
    median_rating, COUNT(m.id) AS movie_count
FROM
    ratings r
        INNER JOIN
...
--------------------------------------------------------------------------------------------*/
-- Type your code below:
SELECT m.title, r.avg_rating AS avg_rating,
CASE 
